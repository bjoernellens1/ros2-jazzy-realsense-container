...
echo "[rtabmap] Vis/MinInliers=${RTABMAP_VIS_MIN_INLIERS}, Vis/MaxFeatures=${RTABMAP_VIS_MAX_FEATURES}, subscribe_imu=true"
echo "[rtabmap] Waiting up to ${RTABMAP_WAIT_TIMEOUT}s for RGB-D + IMU camera topics."
...
while (( SECONDS < deadline )); do
  ...
  if grep -Fxq "${RGB_TOPIC}" <<<"${topics}" &&
     ...
     grep -Fxq "${IMU_TOPIC}" <<<"${topics}"; then
    break
  fi
  sleep 1
done
...
topics="$(ros2 topic list 2>/dev/null || true)"
for topic in "${RGB_TOPIC}" "${DEPTH_TOPIC}" "${CAMERA_INFO_TOPIC}" "${IMU_TOPIC}"; do
